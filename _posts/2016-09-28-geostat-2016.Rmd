...
pkgs = c("sp", "rgdal", "rgrass7")
install_update_load = function(pkgs){
...
The talk touched on the [**gdistance**](https://cran.r-project.org/web/packages/gdistance/) package, and many others. Robert showcased the power of R for understanding major civilisational problems such as the impacts of climate change on agriculture.
His animated global maps of agricultural productivity and precipitation showed how R can scale to tackle large datasets, up to the global level involving spatial and temporal data simultaneously.
...
xFromCol(r)
yFromRow(r)
# xyFromCell(r, 1:ncol(r))
x = 1:ncol(r)
y = 1:nrow(r)
z <- 200 * volcano        # Exaggerate the relief
...
lineid <- surface3d(x, y, z, color = col, back = "lines")["data"]
browsable(tagList(
...
The third day started with a live R demo by Edzer Pebesmer on [space-time data](https://edzer.github.io/UseR2016/).
Refreshingly for a conference primarily on spatial data, it started with an in-depth discussion of time. While base R natively supports temporal units (knowing the difference between days and seconds, for example) it does not know the difference between metres and miles.
...
Day 4 was a highlight for me as I've wanted to learn how to use the **INLA** package for ages. It was explained lucidly by Marta Blangiardo
